...
-   `hydratedNotices`: Notices that were generated by the backend before the block logic run. For example, errors from the server which are not blocking the purchase process but have to be shown to the user.
