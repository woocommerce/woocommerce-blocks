...
-   `canMakePayment` (required): A callback to determine whether the payment method should be available as an option for the shopper. The function will be passed an object containing data about the current cart, and can return return a boolean value or a promise. This allows a payment method to be hidden based on the cart, e.g. if the cart has physical/shippable products (example: `Cash on delivery`). The function Keep in mind this function is only invoked once in the mounting process of the checkout block. Also can be used to disable a payment method based on environment, browser or geolocation restrictions (example: `Apple Pay`). 
