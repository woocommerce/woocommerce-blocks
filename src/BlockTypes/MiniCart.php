...
class MiniCart extends AbstractBlock {
	...
	protected function get_markup( $attributes ) {
		...
		$button_html = '<span class="wc-block-mini-cart__amount" style="' . esc_attr( StyleAttributesUtils::get_font_weight_class_and_style( $attributes )['style'] ) . '">' . esc_html( wp_strip_all_tags( wc_price( $cart_contents_total ) ) ) . '</span>
		' . $this->get_include_tax_label_markup() . '
