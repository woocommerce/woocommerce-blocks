...
class MiniCart extends AbstractBlock {
	...
	/**
	 * Returns whether the cart is empty. If the cart can't be found, it returns
	 * true as well.
	 *
	 * @return boolean Whether the cart is empty.
	 */
	protected function is_cart_empty() {
		$cart = $this->get_cart_instance();
		if ( $cart ) {
			return $cart->get_cart_contents_count() === 0;
		}
		return true;
	}

	/**
	 * Returns the cart icon, depending on whether the cart is empty or not.
	 *
	 * @return string Cart SVG icon.
	 */
	protected function get_mini_cart_icon() {
		if ( $this->is_cart_empty() ) {
			return '<svg class="wc-block-mini-cart__icon" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
