...
class PriceFilterPage {
	...
	async setPriceFilterRange(
		...
		const priceFilterBlock = this.page.locator( blockData.mainClass );
		const minPriceInput = priceFilterBlock.getByRole( 'textbox', {
			name: 'Filter products by minimum price',
		} );
		const maxPriceInput = priceFilterBlock.getByRole( 'textbox', {
	...
	async locateFirstImage() {
		await this.page.waitForLoadState();
		const allProductsBlock = await this.editorUtils.getBlockByName(
			WOOCOMMERCE_ALL_PRODUCTS
		);

		return allProductsBlock.locator( 'img' ).first();
	}

	async locateAllProductsBlock() {
		return await this.editorUtils.getBlockByName(
			WOOCOMMERCE_ALL_PRODUCTS
		);
	}
	...
	async locateAllProducts(
		...
		return this.page.locator( '.products-block-post-template .product' );
	}
