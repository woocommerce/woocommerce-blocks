...
const blockData: BlockData = {
	...
	slug: 'woocommerce/order-confirmation',
	mainClass: '.wp-block-woocommerce-order-confirmation',
	selectors: {
		editor: {
			block: '.wp-block-woocommerce-order-confirmation',
			insertButton: "//button//span[text()='Order Confirmation']",
		},
		frontend: {
			block: '.wp-block-woocommerce-order-confirmation',
		},
...
test.describe( 'Merchant → Order Confirmation', () => {
	...
	test.describe( 'in page editor', () => {
		...
		test( 'renders without crashing and can only be inserted once', async ( {
			...
			const orderConfirmationBlockButton = page.getByRole( 'option', {
				name: blockData.name,
				exact: true,
			} );
			await expect( orderConfirmationBlockButton ).toHaveAttribute(
				'aria-disabled',
				'true'
			);
		} );
