...
describe( 'Store owner can go through store Setup Wizard', () => {
	it( 'can start Setup Wizard when visiting the site for the first time. Skip all other times.', async () => {
		...
		const setupWizardNotice = await Promise.race( [
			...
			page.waitForSelector( '.updated.woocommerce-message.wc-connect', {
				visible: true,
			} ),
		] );
		if ( setupWizardNotice ) {
			...
			const newSetupWizardNotice = await Promise.race( [
				...
				), // resolves without value after 1s
			...
			if ( newSetupWizardNotice ) {
				...
				await Promise.all( [
					...
					page.$eval( '.wc-setup-footer-links', ( elem ) =>
						elem.click()
					),
				...
				await Promise.all( [
					// Click on "Continue" button to move to the next step
					page.click( 'button[name=save_step]', {
						text: 'Continue',
					} ),
				...
				await page.$eval(
					'#wc_recommended_woocommerce_services',
					( elem ) => elem.click()
				);
				...
				await page.waitForSelector( '#wc_recommended_wc_admin', {
					visible: true,
				} );
				await page.$eval( '#wc_recommended_wc_admin', ( elem ) =>
					elem.click()
				);
				...
				await page.waitForSelector( '#wc_recommended_facebook', {
					visible: true,
				} );
